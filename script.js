...
// تعبئة الولايات من بيانات Zimou Express
function initWilayasDropdown() {
  const wilayaSel = document.getElementById('order-wilaya');
  if (!wilayaSel || typeof ZIMOU_PRICES === 'undefined') return;

  wilayaSel.innerHTML = '<option value="">— Choisir la wilaya —</option>' +
    Object.keys(ZIMOU_PRICES).map(w => `<option value="${w}">${w}</option>`).join('');
}

// تحديث قائمة البلديات عند تغيير الولاية
function onWilayaChange(selectedWilaya) {
  const communeSel = document.getElementById('order-commune');
  if (!communeSel) return;

  communeSel.innerHTML = '<option value="">— Choisir la commune —</option>';
  if (selectedWilaya && ZIMOU_COMMUNES[selectedWilaya]) {
    communeSel.innerHTML += ZIMOU_COMMUNES[selectedWilaya].map(c => `<option value="${c}">${c}</option>`).join('');
  }
  updateDeliveryFee();
}

// حساب سعر التوصيل الدقيق
function updateDeliveryFee() {
  ...
  let fee = 0;
  if (wilaya && ZIMOU_PRICES[wilaya]) {
    fee = ZIMOU_PRICES[wilaya];
    // خصم 200 دج في حالة التوصيل للمكتب Stopdesk
    if (deliveryType === 'stopdesk') {
      fee = Math.max(300, fee - 200);
    }
  }

  const subtotal = getCartSubtotal(); // دالة مجموع سلة المشتريات لديك
  const grandTotal = subtotal + fee;

  document.getElementById('display-shipping-fee').textContent = `${fee} DA`;
  document.getElementById('display-grand-total').textContent = `${grandTotal} DA`;
  return { fee, grandTotal };
}

// حفظ الطلب في Supabase
async function submitCustomerOrder() {
  const lName = document.getElementById('order-last-name').value.trim();
  const fName = document.getElementById('order-first-name').value.trim();
  ...
  const { fee, grandTotal } = updateDeliveryFee();

  const orderData = {
    first_name: fName,
    last_name: lName,
    customer_name: `${lName} ${fName}`,
...
document.addEventListener('DOMContentLoaded', initWilayasDropdown);
